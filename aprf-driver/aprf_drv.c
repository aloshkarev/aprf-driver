#include "include/aprf_drv.h"
#include "include/bp-genetlink.h"

#define WARN_QUEUE 100
#define MAX_QUEUE 200

MODULE_AUTHOR("Aleksandr Loshkarev");
MODULE_DESCRIPTION("Software simulator of 802.11 radio(s)");
MODULE_LICENSE("GPL");
...
static int wifi_hwsim_vendor_cmd_test(struct wiphy *wiphy,
                                          ...
                                          const void *data, int data_len)
{
	 ...
	 * here.
	 *
	 * event_idx = 0 (index in wifi_hwsim_vendor_commands)
	 */
...
static struct wiphy_vendor_command wifi_hwsim_vendor_commands[] = {
        {
                .info = { .vendor_id = OUI_QCA,
                        .subcmd = QCA_NL80211_SUBCMD_TEST },
                .flags = WIPHY_VENDOR_CMD_NEED_NETDEV,
                .doit = wifi_hwsim_vendor_cmd_test,
...
static const struct nl80211_vendor_cmd_info wifi_hwsim_vendor_events[] = {
...
static struct platform_driver wifi_hwsim_driver = {
        .driver = {
                .name = "aprf_drv",
        },
};
...
static const struct rhashtable_params hwsim_rht_params = {
        ...
        .key_len = ETH_ALEN,
        .key_offset = offsetof(struct wifi_hwsim_data, addresses[1]),
        .head_offset = offsetof(struct wifi_hwsim_data, rht),
};
...
static int hwsim_tx_virtio(struct wifi_hwsim_data *data,
                           struct sk_buff *skb)
{
...
extern int hwsim_tx_virtio(struct wifi_hwsim_data *data,
			   struct sk_buff *skb);
...
static void wifi_hwsim_tx_frame(struct ieee80211_hw *hw,
                                    struct sk_buff *skb,
...
{
    struct wifi_hwsim_data *data = dat;
    struct hwsim_vif_priv *vp = (void *)vif->drv_priv;
    struct sk_buff *skb;
    ...
    rcu_read_lock();
    wifi_hwsim_tx_frame(data->hw, skb,
                            rcu_dereference(vif->chanctx_conf)->def.chan);
    rcu_read_unlock();
}

static void hwsim_send_nullfunc(struct wifi_hwsim_data *data, u8 *mac,
...
static inline u64 wifi_hwsim_get_tsf_raw(void)
{
    return ktime_to_us(ktime_get_real());
}

static __le64 __wifi_hwsim_get_tsf(struct wifi_hwsim_data *data)
{
    u64 now = wifi_hwsim_get_tsf_raw();
    return cpu_to_le64(now + data->tsf_offset);
}

static u64 wifi_hwsim_get_tsf(struct ieee80211_hw *hw,
                                  struct ieee80211_vif *vif)
{
    struct wifi_hwsim_data *data = hw->priv;
    return le64_to_cpu(__wifi_hwsim_get_tsf(data));
}

static void wifi_hwsim_set_tsf(struct ieee80211_hw *hw,
                                   struct ieee80211_vif *vif, u64 tsf)
{
    struct wifi_hwsim_data *data = hw->priv;
    u64 now = wifi_hwsim_get_tsf(hw, vif);
...
static void wifi_hwsim_monitor_rx(struct ieee80211_hw *hw,
                                      ...
                                      struct ieee80211_channel *chan)
{
    ...
    hdr->rt_tsft = __wifi_hwsim_get_tsf(data);
    hdr->rt_flags = 0;
...
static void wifi_hwsim_monitor_ack(struct ieee80211_channel *chan,
                                       const u8 *addr)
{
    struct sk_buff *skb;
...
struct wifi_hwsim_addr_match_data {
    u8 addr[ETH_ALEN];
    bool ret;
};

static void wifi_hwsim_addr_iter(void *data, u8 *mac,
                                     struct ieee80211_vif *vif)
{
    struct wifi_hwsim_addr_match_data *md = data;

    if (memcmp(mac, md->addr, ETH_ALEN) == 0)
        md->ret = true;
}

static bool wifi_hwsim_addr_match(struct wifi_hwsim_data *data,
                                      const u8 *addr)
{
    struct wifi_hwsim_addr_match_data md = {
            .ret = false,
    };
    ...
    ieee80211_iterate_active_interfaces_atomic(data->hw,
                                               ...
                                               wifi_hwsim_addr_iter,
                                               &md);

    return md.ret;
}

static bool hwsim_ps_rx_ok(struct wifi_hwsim_data *data,
                           struct sk_buff *skb)
{
    switch (data->ps) {
        case PS_DISABLED:
		 ...
		 * PS-Poll */
            if (data->ps_poll_pending &&
                wifi_hwsim_addr_match(data, skb->data + 4)) {
...
static int hwsim_unicast_netgroup(struct wifi_hwsim_data *data,
...
static void wifi_hwsim_config_mac_nl(struct ieee80211_hw *hw,
                                         const u8 *addr, bool add)
{
    ...
    if (!msg_head) {
        pr_debug("aprf_drv: problem with msg_head\n");
...
static inline u16 trans_tx_rate_flags_ieee2hwsim(struct ieee80211_tx_rate *rate)
{
    ...
    if (rate->flags & IEEE80211_TX_RC_USE_RTS_CTS)
        result |= WIFI_HWSIM_TX_RC_USE_RTS_CTS;
    if (rate->flags & IEEE80211_TX_RC_USE_CTS_PROTECT)
        result |= WIFI_HWSIM_TX_RC_USE_CTS_PROTECT;
    if (rate->flags & IEEE80211_TX_RC_USE_SHORT_PREAMBLE)
        result |= WIFI_HWSIM_TX_RC_USE_SHORT_PREAMBLE;
    if (rate->flags & IEEE80211_TX_RC_MCS)
        result |= WIFI_HWSIM_TX_RC_MCS;
    if (rate->flags & IEEE80211_TX_RC_GREEN_FIELD)
        result |= WIFI_HWSIM_TX_RC_GREEN_FIELD;
    if (rate->flags & IEEE80211_TX_RC_40_MHZ_WIDTH)
        result |= WIFI_HWSIM_TX_RC_40_MHZ_WIDTH;
    if (rate->flags & IEEE80211_TX_RC_DUP_DATA)
        result |= WIFI_HWSIM_TX_RC_DUP_DATA;
    if (rate->flags & IEEE80211_TX_RC_SHORT_GI)
        result |= WIFI_HWSIM_TX_RC_SHORT_GI;
    if (rate->flags & IEEE80211_TX_RC_VHT_MCS)
        result |= WIFI_HWSIM_TX_RC_VHT_MCS;
    if (rate->flags & IEEE80211_TX_RC_80_MHZ_WIDTH)
        result |= WIFI_HWSIM_TX_RC_80_MHZ_WIDTH;
    if (rate->flags & IEEE80211_TX_RC_160_MHZ_WIDTH)
        result |= WIFI_HWSIM_TX_RC_160_MHZ_WIDTH;

    return result;
}

static void wifi_hwsim_tx_frame_nl(struct ieee80211_hw *hw,
                                       ...
                                       struct ieee80211_channel *channel)
{
    ...
    nlmsg_free(skb);
    err_free_txskb:
    pr_debug("aprf_drv error occurred in %s\n", __func__);
...
static void wifi_hwsim_tx_iter(void *_data, u8 *addr,
                                   struct ieee80211_vif *vif)
{
    struct tx_iter_data *data = _data;
#if LINUX_VERSION_PATCHLEVEL == 15
    if (!vif->chanctx_conf)
        return;
#elif LINUX_VERSION_PATCHLEVEL == 19
...
static void wifi_hwsim_add_vendor_rtap(struct sk_buff *skb)
{
    /*
	...
	/* make sure to clear padding, wifi doesn't */
...
static bool wifi_hwsim_tx_frame_no_nl(struct ieee80211_hw *hw,
                                          struct sk_buff *skb,
                                          struct ieee80211_channel *chan)
{
    struct wifi_hwsim_data *data = hw->priv, *data2;
    bool ack = false;
    ...
    list_for_each_entry(data2, &hwsim_radios, list) {
        ...
        if (!hwsim_chans_compat(chan, data2->tmp_chan) &&
            ...
            ieee80211_iterate_active_interfaces_atomic(
                    ...
                    wifi_hwsim_tx_iter, &tx_iter_data);
        ...
        if (wifi_hwsim_addr_match(data2, hdr->addr1))
            ack = true;
        ...
        wifi_hwsim_add_vendor_rtap(nskb);
...
static void wifi_hwsim_tx(struct ieee80211_hw *hw,
                              ...
                              struct sk_buff *skb)
{
    ...
    if (skb->len >= 24 + 8 &&
        ...
        if (txrate)
            bitrate = txrate->bitrate;
        ts = wifi_hwsim_get_tsf_raw();
        mgmt->u.probe_resp.timestamp =
                        cpu_to_le64(ts + data->tsf_offset +
                                    24 * 8 * 10 / bitrate);
    }

    wifi_hwsim_monitor_rx(hw, skb, channel);

    /* wmediumd mode check */
    _portid = READ_ONCE(data->wmediumd);

    if (_portid || hwsim_virtio_enabled)
        return wifi_hwsim_tx_frame_nl(hw, skb, _portid, channel);

    /* NO wmediumd detected, perfect medium simulation */
    data->tx_pkts++;
    data->tx_bytes += skb->len;
    ack = wifi_hwsim_tx_frame_no_nl(hw, skb, channel);

    if (ack && skb->len >= 16)
        wifi_hwsim_monitor_ack(channel, hdr->addr2);
...
static int wifi_hwsim_start(struct ieee80211_hw *hw)
{
    struct wifi_hwsim_data *data = hw->priv;
...
static void wifi_hwsim_stop(struct ieee80211_hw *hw)
{
    struct wifi_hwsim_data *data = hw->priv;
...
static int wifi_hwsim_add_interface(struct ieee80211_hw *hw,
                                        struct ieee80211_vif *vif)
{
    ...
    if (vif->type != NL80211_IFTYPE_MONITOR)
        wifi_hwsim_config_mac_nl(hw, vif->addr, true);
...
static int wifi_hwsim_change_interface(struct ieee80211_hw *hw,
                                           struct ieee80211_vif *vif,
...
static void wifi_hwsim_remove_interface(
        struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
    ...
    if (vif->type != NL80211_IFTYPE_MONITOR)
        wifi_hwsim_config_mac_nl(hw, vif->addr, false);
}
                                    ...
                                    struct ieee80211_channel *chan)
{
    ...
    wifi_hwsim_monitor_rx(hw, skb, chan);

    if (_pid || hwsim_virtio_enabled)
        return wifi_hwsim_tx_frame_nl(hw, skb, _pid, chan);

    data->tx_pkts++;
    data->tx_bytes += skb->len;
    wifi_hwsim_tx_frame_no_nl(hw, skb, chan);
    dev_kfree_skb(skb);
}

static void wifi_hwsim_beacon_tx(void *arg, u8 *mac,
                                     struct ieee80211_vif *vif)
{
    struct wifi_hwsim_data *data = arg;
    ...
    mgmt = (struct ieee80211_mgmt *) skb->data;
    /* fake header transmission time */
    data->abs_bcn_ts = wifi_hwsim_get_tsf_raw();
...
wifi_hwsim_beacon(struct hrtimer *timer)
{
    struct wifi_hwsim_data *data =
            container_of(timer, struct wifi_hwsim_data, beacon_timer);
    struct ieee80211_hw *hw = data->hw;
...
static int wifi_hwsim_config(struct ieee80211_hw *hw, u32 changed)
{
    struct wifi_hwsim_data *data = hw->priv;
    ...
    if (!data->started || !data->beacon_int)
        ...
        u64 tsf = wifi_hwsim_get_tsf(hw, NULL);
        u32 bcn_int = data->beacon_int;
...
static void wifi_hwsim_configure_filter(struct ieee80211_hw *hw,
...
static void wifi_hwsim_bcn_en_iter(void *data, u8 *mac,
                                       struct ieee80211_vif *vif)
{
...
static void wifi_hwsim_bss_info_changed(struct ieee80211_hw *hw,
                                            struct ieee80211_vif *vif,
...
static void wifi_hwsim_sta_rc_update(struct ieee80211_hw *hw,
			     struct ieee80211_vif *vif,
...
// 		}
		...
		//      vif->addr, link_id, sta->addr, bw,/* sta->deflink.bandwidth*/0,
...
static int wifi_hwsim_sta_add(struct ieee80211_hw *hw,
                                  struct ieee80211_vif *vif,
                                  struct ieee80211_sta *sta)
{
    hwsim_check_magic(vif);
...
static int wifi_hwsim_sta_remove(struct ieee80211_hw *hw,
                                     struct ieee80211_vif *vif,
                                     struct ieee80211_sta *sta)
{
    hwsim_check_magic(vif);
...
static int wifi_hwsim_sta_state(struct ieee80211_hw *hw,
				    struct ieee80211_vif *vif,
				    struct ieee80211_sta *sta,
				    ...
				    enum ieee80211_sta_state new_state)
{
	if (new_state == IEEE80211_STA_NOTEXIST)
		return wifi_hwsim_sta_remove(hw, vif, sta);

	if (old_state == IEEE80211_STA_NOTEXIST)
		return wifi_hwsim_sta_add(hw, vif, sta);
...
static void wifi_hwsim_sta_notify(struct ieee80211_hw *hw,
                                      struct ieee80211_vif *vif,
...
static int wifi_hwsim_set_tim(struct ieee80211_hw *hw,
                                  struct ieee80211_sta *sta,
                                  bool set)
{
...
static int wifi_hwsim_get_survey(struct ieee80211_hw *hw, int idx,
                                     struct survey_info *survey)
{
    struct wifi_hwsim_data *hwsim = hw->priv;
...
static int wifi_hwsim_testmode_cmd(struct ieee80211_hw *hw,
				       struct ieee80211_vif *vif,
				       void *data, int len)
{
	struct wifi_hwsim_data *hwsim = hw->priv;
...
static int wifi_hwsim_ampdu_action(struct ieee80211_hw *hw,
                                       struct ieee80211_vif *vif,
...
static void wifi_hwsim_flush(struct ieee80211_hw *hw,
                                 struct ieee80211_vif *vif,
...
{
    struct wifi_hwsim_data *hwsim =
            container_of(work, struct wifi_hwsim_data, hw_scan.work);
    ...
    if (hwsim->scan_chan_idx >= req->n_channels) {
        ...
        hwsim->tmp_chan = NULL;
        mutex_unlock(&hwsim->mutex);
        wifi_hwsim_config_mac_nl(hwsim->hw, hwsim->scan_addr,
                                     false);
        return;
    }
    ...
    if (hwsim->tmp_chan->flags & (IEEE80211_CHAN_NO_IR |
        ...
        for (i = 0; i < req->n_ssids; i++) {
            ...
            local_bh_disable();
            wifi_hwsim_tx_frame(hwsim->hw, probe,
                                    hwsim->tmp_chan);
            rcu_read_unlock();
            local_bh_enable();
        }
...
static int wifi_hwsim_hw_scan(struct ieee80211_hw *hw,
                                  struct ieee80211_vif *vif,
                                  struct ieee80211_scan_request *hw_req)
{
    ...
    wifi_hwsim_config_mac_nl(hw, hwsim->scan_addr, true);
...
static void wifi_hwsim_cancel_hw_scan(struct ieee80211_hw *hw,
                                          struct ieee80211_vif *vif)
{
    struct wifi_hwsim_data *hwsim = hw->priv;
    struct cfg80211_scan_info info = {
...
static void wifi_hwsim_sw_scan(struct ieee80211_hw *hw,
                                   struct ieee80211_vif *vif,
                                   const u8 *mac_addr)
{
    struct wifi_hwsim_data *hwsim = hw->priv;
...
static void wifi_hwsim_sw_scan_complete(struct ieee80211_hw *hw,
                                            struct ieee80211_vif *vif)
{
    struct wifi_hwsim_data *hwsim = hw->priv;
    ...
    wifi_hwsim_config_mac_nl(hw, hwsim->scan_addr, false);
...
{
    struct wifi_hwsim_data *hwsim =
            container_of(work, struct wifi_hwsim_data, roc_start.work);
...
{
    struct wifi_hwsim_data *hwsim =
            container_of(work, struct wifi_hwsim_data, roc_done.work);
...
static int wifi_hwsim_roc(struct ieee80211_hw *hw,
                              struct ieee80211_vif *vif,
                              struct ieee80211_channel *chan,
                              int duration,
...
static int wifi_hwsim_croc(struct ieee80211_hw *hw,
                               struct ieee80211_vif *vif)
{
    struct wifi_hwsim_data *hwsim = hw->priv;
...
static int wifi_hwsim_add_chanctx(struct ieee80211_hw *hw,
...
static void wifi_hwsim_remove_chanctx(struct ieee80211_hw *hw,
                                          struct ieee80211_chanctx_conf *ctx)
{
    struct wifi_hwsim_data *hwsim = hw->priv;
...
static void wifi_hwsim_change_chanctx(struct ieee80211_hw *hw,
                                          struct ieee80211_chanctx_conf *ctx,
                                          u32 changed)
{
    struct wifi_hwsim_data *hwsim = hw->priv;
...
static int wifi_hwsim_assign_vif_chanctx(struct ieee80211_hw *hw,
                                             struct ieee80211_vif *vif,
                                             struct ieee80211_chanctx_conf *ctx)
{
    hwsim_check_magic(vif);
...
static void wifi_hwsim_unassign_vif_chanctx(struct ieee80211_hw *hw,
                                                struct ieee80211_vif *vif,
                                                struct ieee80211_chanctx_conf *ctx)
{
    hwsim_check_magic(vif);
...
static const char wifi_hwsim_gstrings_stats[][ETH_GSTRING_LEN] = {
        "tx_pkts_nic",
        "tx_bytes_nic",
        "rx_pkts_nic",
        "rx_bytes_nic",
...
#define WIFI_HWSIM_SSTATS_LEN ARRAY_SIZE(wifi_hwsim_gstrings_stats)

static void wifi_hwsim_get_et_strings(struct ieee80211_hw *hw,
                                          struct ieee80211_vif *vif,
                                          u32 sset, u8 *data)
{
    if (sset == ETH_SS_STATS)
        memcpy(data, *wifi_hwsim_gstrings_stats,
               sizeof(wifi_hwsim_gstrings_stats));
}

static int wifi_hwsim_get_et_sset_count(struct ieee80211_hw *hw,
                                            struct ieee80211_vif *vif, int sset)
{
    if (sset == ETH_SS_STATS)
        return WIFI_HWSIM_SSTATS_LEN;
    return 0;
}

static void wifi_hwsim_get_et_stats(struct ieee80211_hw *hw,
                                        struct ieee80211_vif *vif,
                                        struct ethtool_stats *stats, u64 *data)
{
    struct wifi_hwsim_data *ar = hw->priv;
    int i = 0;
    ...
    WARN_ON(i != WIFI_HWSIM_SSTATS_LEN);
}

static int wifi_hwsim_tx_last_beacon(struct ieee80211_hw *hw)
{
    return 1;
}

static int wifi_hwsim_set_rts_threshold(struct ieee80211_hw *hw, u32 value)
{
	return -EOPNOTSUPP;
}
//
// static int wifi_hwsim_change_vif_links(struct ieee80211_hw *hw,
// 					   struct ieee80211_vif *vif,
// 					   u16 old_links, u16 new_links,
// 					   struct ieee80211_bss_conf *old[15])
// {
...
// 		add |= BIT(0);
//
// 	for_each_set_bit(i, &rem, 15)
// 		wifi_hwsim_config_mac_nl(hw, old[i]->addr, false);
//
// 	for_each_set_bit(i, &add, 15) {
...
// 			continue;
//
// 		wifi_hwsim_config_mac_nl(hw, link_conf->addr, true);
// 	}
//
// 	return 0;
// }
//
// static int wifi_hwsim_change_sta_links(struct ieee80211_hw *hw,
// 					   struct ieee80211_vif *vif,
// 					   struct ieee80211_sta *sta,
// 					   u16 old_links, u16 new_links)
// {
...
static int wifi_hwsim_send_pmsr_ftm_request_peer(struct sk_buff *msg,
...
static int wifi_hwsim_send_pmsr_request_peer(struct sk_buff *msg,
						 struct cfg80211_pmsr_request_peer *request)
{
	...
	err = wifi_hwsim_send_pmsr_ftm_request_peer(msg, &request->ftm);
	if (err)
		return err;
...
static int wifi_hwsim_send_pmsr_request(struct sk_buff *msg,
					    struct cfg80211_pmsr_request *request)
{
	...
	for(i = 0; i < request->n_peers; i++) {
		err = wifi_hwsim_send_pmsr_request_peer(msg, &request->peers[i]);
		if (err)
			return err;
	}
...
static int wifi_hwsim_start_pmsr(struct ieee80211_hw *hw,
				     struct ieee80211_vif *vif,
				     struct cfg80211_pmsr_request *request)
{
	struct wifi_hwsim_data *data;
	...
	err = wifi_hwsim_send_pmsr_request(skb, request);
	if (err)
		goto out_free;
...
static void wifi_hwsim_abort_pmsr(struct ieee80211_hw *hw,
				      struct ieee80211_vif *vif,
				      struct cfg80211_pmsr_request *request)
{
	struct wifi_hwsim_data *data;
...
static int wifi_hwsim_parse_rate_info(struct nlattr *rateattr,
...
static int wifi_hwsim_parse_ftm_result(struct nlattr *ftm,
					   ...
					   struct genl_info *info)
{
	...
	if (tb[NL80211_PMSR_FTM_RESP_ATTR_TX_RATE]) {
		...
		ret = wifi_hwsim_parse_rate_info(tb[NL80211_PMSR_FTM_RESP_ATTR_TX_RATE],
						     &result->tx_rate, info);
		if (ret)
			return ret;
	}
	...
	if (tb[NL80211_PMSR_FTM_RESP_ATTR_RX_RATE]) {
		...
		ret = wifi_hwsim_parse_rate_info(tb[NL80211_PMSR_FTM_RESP_ATTR_RX_RATE],
						     &result->rx_rate, info);
		if (ret)
			return ret;
	}
...
static int wifi_hwsim_parse_pmsr_resp(struct nlattr *resp,
					  ...
					  struct genl_info *info)
{
	...
	nla_for_each_nested(pmsr, tb[NL80211_PMSR_RESP_ATTR_DATA], rem) {
		...
		case NL80211_PMSR_TYPE_FTM:
			...
			ret = wifi_hwsim_parse_ftm_result(pmsr, &result->ftm, info);
			if (ret)
				return ret;
			break;
		default:
...
static int wifi_hwsim_parse_pmsr_result(struct nlattr *peer,
					    struct cfg80211_pmsr_result *result,
					    struct genl_info *info)
{
	...
	if (tb[NL80211_PMSR_PEER_ATTR_RESP]) {
		ret = wifi_hwsim_parse_pmsr_resp(tb[NL80211_PMSR_PEER_ATTR_RESP], result, info);
		if (ret)
			return ret;
	}
...
static struct wifi_hwsim_data *get_hwsim_data_ref_from_addr(const u8 *addr)
{
...
static int hwsim_pmsr_report_nl(struct sk_buff *msg, struct genl_info *info)
{
	...
	nla_for_each_nested(peer, peers, rem) {
		...
		err = wifi_hwsim_parse_pmsr_result(peer, &result, info);
		if (err)
			goto out;
...
#define HWSIM_COMMON_OPS					\
	.tx = wifi_hwsim_tx,				\
	.start = wifi_hwsim_start,				\
	.stop = wifi_hwsim_stop,				\
	.add_interface = wifi_hwsim_add_interface,		\
	.change_interface = wifi_hwsim_change_interface,	\
	.remove_interface = wifi_hwsim_remove_interface,	\
	.config = wifi_hwsim_config,			\
	.configure_filter = wifi_hwsim_configure_filter,	\
	.bss_info_changed = wifi_hwsim_bss_info_changed,	\
	.tx_last_beacon = wifi_hwsim_tx_last_beacon,	\
	.sta_add = wifi_hwsim_sta_add,			\
	.sta_remove = wifi_hwsim_sta_remove,		\
	.sta_notify = wifi_hwsim_sta_notify,		\
	.set_tim = wifi_hwsim_set_tim,			\
	.conf_tx = wifi_hwsim_conf_tx,			\
	.get_survey = wifi_hwsim_get_survey,		\
	CFG80211_TESTMODE_CMD(wifi_hwsim_testmode_cmd)	\
	.ampdu_action = wifi_hwsim_ampdu_action,		\
	.flush = wifi_hwsim_flush,				\
	.get_tsf = wifi_hwsim_get_tsf,			\
	.set_tsf = wifi_hwsim_set_tsf,			\
	.get_et_sset_count = wifi_hwsim_get_et_sset_count,	\
	.get_et_stats = wifi_hwsim_get_et_stats,		\
	.get_et_strings = wifi_hwsim_get_et_strings,

static const struct ieee80211_ops wifi_hwsim_ops = {
        HWSIM_COMMON_OPS
        .sw_scan_start = wifi_hwsim_sw_scan,
        .sw_scan_complete = wifi_hwsim_sw_scan_complete,
};
...
static void wifi_hwsim_he_capab(struct ieee80211_supported_band *sband)
{
    u16 n_iftype_data;
...
#define HWSIM_COMMON_OPS					\
	...
	.config = wifi_hwsim_config,			\
	.configure_filter = wifi_hwsim_configure_filter,	\
	/*.vif_cfg_changed = wifi_hwsim_vif_info_changed,	\
	.link_info_changed = wifi_hwsim_link_info_changed,  \*/\
	.tx_last_beacon = wifi_hwsim_tx_last_beacon,	\
	.sta_notify = wifi_hwsim_sta_notify,		\
	.sta_rc_update = wifi_hwsim_sta_rc_update,		\
	.conf_tx = wifi_hwsim_conf_tx,			\
	.get_survey = wifi_hwsim_get_survey,		\
	CFG80211_TESTMODE_CMD(wifi_hwsim_testmode_cmd)	\
	.ampdu_action = wifi_hwsim_ampdu_action,		\
	...
	.get_et_stats = wifi_hwsim_get_et_stats,		\
	.get_et_strings = wifi_hwsim_get_et_strings,	\
	.start_pmsr = wifi_hwsim_start_pmsr,		\
	.abort_pmsr = wifi_hwsim_abort_pmsr,

#define HWSIM_CHANCTX_OPS					\
	.hw_scan = wifi_hwsim_hw_scan,			\
	.cancel_hw_scan = wifi_hwsim_cancel_hw_scan,	\
	.remain_on_channel = wifi_hwsim_roc,		\
	.cancel_remain_on_channel = wifi_hwsim_croc,	\
	.add_chanctx = wifi_hwsim_add_chanctx,		\
	.remove_chanctx = wifi_hwsim_remove_chanctx,	\
	.change_chanctx = wifi_hwsim_change_chanctx,	\
	.assign_vif_chanctx = wifi_hwsim_assign_vif_chanctx,\
	.unassign_vif_chanctx = wifi_hwsim_unassign_vif_chanctx,
...
static const struct ieee80211_ops wifi_hwsim_mchan_ops = {
	HWSIM_COMMON_OPS
	HWSIM_NON_MLO_OPS
	HWSIM_CHANCTX_OPS
};

static const struct ieee80211_ops wifi_hwsim_mlo_ops = {
	HWSIM_COMMON_OPS
	HWSIM_CHANCTX_OPS
	.set_rts_threshold = wifi_hwsim_set_rts_threshold,
	// .change_vif_links = wifi_hwsim_change_vif_links,
	// .change_sta_links = wifi_hwsim_change_sta_links,
	.sta_state = wifi_hwsim_sta_state,
};

static int wifi_hwsim_new_radio(struct genl_info *info,
                                    struct hwsim_new_radio_params *param)
{
    int err;
    u8 addr[ETH_ALEN];
    ...
    const struct ieee80211_ops *ops = &wifi_hwsim_ops;
    struct net *net;
    int idx, i;
    int n_limits = 0;
    ...
    if (param->mlo)
		ops = &wifi_hwsim_mlo_ops;
    else if (param->use_chanctx)
        ops = &wifi_hwsim_mchan_ops;
    hw = ieee80211_alloc_hw_nm(sizeof(*data), ops, param->hwname);
    if (!hw) {
        pr_debug("aprf_drv: ieee80211_alloc_hw failed\n");
        err = -ENOMEM;
        goto failed;
    }
    ...
    data->dev = device_create(hwsim_class, NULL, 0, hw, "wemu%d", idx);
    if (IS_ERR(data->dev)) {
        printk(KERN_DEBUG
                       "aprf_drv: device_create failed (%ld)\n",
               PTR_ERR(data->dev));
        err = -ENOMEM;
        goto failed_drvdata;
    }
    data->dev->driver = &wifi_hwsim_driver.driver;
    err = device_bind_driver(data->dev);
    if (err != 0) {
        pr_debug("aprf_drv: device_bind_driver failed (%d)\n",
                 err);
        goto failed_bind;
    }
    ...
    /* ask wifi to reserve space for magic */
    ...
    hw->wiphy->vendor_commands = wifi_hwsim_vendor_commands;
    hw->wiphy->n_vendor_commands =
            ARRAY_SIZE(wifi_hwsim_vendor_commands);
    hw->wiphy->vendor_events = wifi_hwsim_vendor_events;
    hw->wiphy->n_vendor_events = ARRAY_SIZE(wifi_hwsim_vendor_events);
    ...
    data->beacon_timer.function = wifi_hwsim_beacon;

    err = ieee80211_register_hw(hw);
    if (err < 0) {
        pr_debug("aprf_drv: ieee80211_register_hw failed (%d)\n",
                 err);
        goto failed_hw;
    }
    ...
    data->debugfs = debugfs_create_dir("wemu", hw->wiphy->debugfsdir);
...
static void wifi_hwsim_del_radio(struct wifi_hwsim_data *data,
                                     const char *hwname,
                                     struct genl_info *info)
{
...
static int wifi_hwsim_get_radio(struct sk_buff *skb,
                                    struct wifi_hwsim_data *data,
                                    u32 portid, u32 seq,
...
static void wifi_hwsim_free(void)
{
    struct wifi_hwsim_data *data;
    ...
    while ((data = list_first_entry_or_null(&hwsim_radios,
        ...
        list_del(&data->list);
        spin_unlock_bh(&hwsim_radio_lock);
        wifi_hwsim_del_radio(data, wiphy_name(data->hw->wiphy),
                                 NULL);
        spin_lock_bh(&hwsim_radio_lock);
    }
    spin_unlock_bh(&hwsim_radio_lock);
                                           ...
                                           struct genl_info *info)
{
    ...
    if (!(hwsim_flags & HWSIM_TX_CTL_NO_ACK) &&
        ...
        if (skb->len >= 16) {
            ...
            wifi_hwsim_monitor_ack(data2->channel,
                                       hdr->addr2);
        }
        txi->flags |= IEEE80211_TX_STAT_ACK;
    }
                                      ...
                                      struct genl_info *info)
{
    ...
    pr_debug("aprf_drv: received a REGISTER, "
             "switching to eltex_wmediumd mode with pid %d\n", info->snd_portid);
...
static int hwsim_new_radio_nl(struct sk_buff *msg, struct genl_info *info)
{
    ...
    ret = wifi_hwsim_new_radio(info, &param);
    kfree(hwname);
    return ret;
}
...
static int hwsim_get_radio_nl(struct sk_buff *msg, struct genl_info *info)
{
    ...
    list_for_each_entry(data, &hwsim_radios, list) {
        ...
        res = wifi_hwsim_get_radio(skb, data, info->snd_portid,
                                       info->snd_seq, NULL, 0);
        if (res < 0) {
            nlmsg_free(skb);
            goto out_err;
                               ...
                               struct netlink_callback *cb)
{
    ...
    struct wifi_hwsim_data *data = NULL;
    int res = 0;
    void *hdr;
    ...
    list_for_each_entry(data, &hwsim_radios, list) {
        ...
        res = wifi_hwsim_get_radio(skb, data,
...
{
    struct wifi_hwsim_data *entry, *tmp;
    LIST_HEAD(list);
    ...
    list_for_each_entry_safe(entry, tmp, &list, list) {
        list_del(&entry->list);
        wifi_hwsim_del_radio(entry, wiphy_name(entry->hw->wiphy),
                                 NULL);
    }
}

static int wifi_hwsim_netlink_notify(struct notifier_block *nb,
                                         unsigned long state,
                                         void *_notify)
{
    ...
    if (notify->portid == hwsim_net_get_wmediumd(notify->net)) {
        printk(KERN_INFO "aprf_drv: eltex_wmediumd released netlink"
...
static struct notifier_block hwsim_netlink_notifier = {
        .notifier_call = wifi_hwsim_netlink_notify,
};
...
static int __init hwsim_init_netlink(void)
{
    ...
    printk(KERN_INFO "aprf_drv: initializing netlink\n");
...
{
    struct wifi_hwsim_data *data, *tmp;
    LIST_HEAD(list);
...
/* APRF_DRV virtio device id table */
...
static int __init init_wifi_hwsim(void)
{
    int i, err;
    ...
    err = platform_driver_register(&wifi_hwsim_driver);
    if (err)
    ...
    hwsim_class = class_create(THIS_MODULE, "aprf_drv");
    ...
    for (i = 0; i < radios; i++) {
        ...
        err = wifi_hwsim_new_radio(NULL, &param);
        if (err < 0)
            goto out_free_radios;
    }

    hwsim_mon = alloc_netdev(0, "wemu%d", NET_NAME_UNKNOWN,
                             hwsim_mon_setup);
    if (hwsim_mon == NULL) {
        err = -ENOMEM;
        goto out_free_radios;
    ...
    platform_driver_unregister(&wifi_hwsim_driver);
...
module_init(init_wifi_hwsim);

static void __exit exit_wifi_hwsim(void)
{
    pr_debug("aprf_drv: unregister radios\n");
